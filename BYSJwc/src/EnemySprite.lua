...
function EnemySprite:init(enemytype)
    self.type = enemytype.type                  -- 类型
    self.HP = enemytype.HP                      -- 血量
    self.power = enemytype.power                -- 爆炸范围
    ...
    local scaleNumber = enemyBulletConfig[enemytype.type].sizeOfScale
    self:setScale(scaleNumber)
    ...
    self:setPhysicsBody(cc.PhysicsBody:createBox(cc.size(self.size.width*scaleNumber, 
        self.size.height*scaleNumber)))
